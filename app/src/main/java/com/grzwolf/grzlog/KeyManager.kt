...
//   is safe, but is too slow for larger texts
...
//   --> the password is set by user
//   --> this password is then encrypted asymmetrically with the app's KeyStore public key
//   --> this encrypted password is stored in the app's preferences
//   --> if the password is needed to symmetrically encrypt/decrypt app data, it is read from
...
class KeyManager(private val context: Context, keyStoreAlias: String, private val appName: String) {
    ...
    fun isEncryptedWithPub(textToCheck: String): Boolean {
        try {
            ...
            // if logic ends here --> no encryption or something went dramatically wrong
            return false
        }
    }

    // encrypt a given String (here: mostly GrzLog encryption pwd) with the app's public key
    ...
    // decrypt a given String (here: mostly GrzLog encryption pwd) with the app's private key
        ...
        try {
            ...
            if (e.message?.contains("Empty key") ?: true) {
                (context as Activity).runOnUiThread {
                    centeredToast(
                        context,
                        "decryption failed: no password",
                        5000
                    )
                }
                val sharedPref = PreferenceManager.getDefaultSharedPreferences(context)
                sharedPref.edit().let {
                    it.putBoolean("encryptProtectedFolders", false)
                    it.apply()
                }
            }
            ...
            if (e.message?.contains("Unsupported key size:") ?: true) {
                (context as Activity).runOnUiThread {
                    centeredToast(
                        context,
                        "decryption failed: password wrong size",
                        5000
                    )
                }
                val sharedPref = PreferenceManager.getDefaultSharedPreferences(context)
    ...
    companion object {
        ...
        // check an arbitrary String whether it is perhaps AES / GCM encrypted
        // note: this makes sure, that a text inside GrzLog behaves like an AES / GCM encrypted text
        fun isTextEncryptedGCM(text: String): Boolean {
            var retVal = false
            val parts = text.split(":")                            // GCM separates cipher text and auth tag with ':'
            if (parts.size == 2) {                                 // makes sure, a GCM encryption contains exactly one ':'
                var wsCount = 0                                    // Java Base64.encodeToString never contains ' ', allowed: A-Z a-z 0-9 + /
                text.forEach { char ->
                    if (char.code == 32) {
                        wsCount++                                  // count all ' ' in text
                    }
                }
                val atl = parts[1].length                          // standard Java GCM authentication tag length = 16 PLUS ...
                val lfFound = text.endsWith('\n')                  // ... a trailing '\n'
                val charEndMinus1atl = text[text.length - atl - 1] // divides cipher text and GCM auth tag, supposed to be ':'
                val charEndMinus2atl = text[text.length - atl - 2] // last char of cipher text, supposed to be '\n'
                // finally do the checks
                if ( wsCount == 0 && atl == 17 && lfFound && charEndMinus1atl.equals(':') && charEndMinus2atl.equals('\n') == true ) {
                    retVal = true
                }
            }
            return retVal
